...
$order_id=$row['id'];
$massage="Order is out for delivery for Order request id :".$order_id." & Date ".$order_date;
if($row['user_type'] == "Android")
{
    ...
    if ($dataj->success == 1) {
        $update=mysqli_query($con,"update tbl_order_homedelivery set order_status=2 WHERE id='$order_id'");
        if($update){
